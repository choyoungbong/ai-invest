...
[A단계] RSI 정확도 향상 + ATR 변동성 필터

[ChatGPT 자문 반영]
  거래대금 절대값 필터 (신규):
    - 당일 거래대금 < MIN_TRADING_VALUE(기본 30억) 종목 차단
    - 거래대금 배수(VOLUME_MULTIPLIER) 필터와 병행 적용
    - 이유: 소형주 급락의 주요 원인이 거래대금 부족
             거래대금이 적으면 매도 시 슬리피지가 커지고 손절 실효성 하락
    - 환경변수: MIN_TRADING_VALUE (기본 3,000,000,000 = 30억)

  갭상승 필터 (신규):
    - 시초가 / 전일 종가 대비 갭상승률 > MAX_GAP_UP_PCT(기본 3.5%) 종목 차단
    - 이유: 갭상승 후 되돌림은 자동매매 손절의 대표 패턴
             이미 많이 오른 상태에서 진입하면 손익비 악화
    - 환경변수: MAX_GAP_UP_PCT (기본 3.5%)
                GAP_UP_FILTER_ENABLED (기본 true)
"""
import logging
import os
import uuid
...
ATR_FILTER_MAX_PCT = float(os.getenv("ATR_FILTER_MAX_PCT", "3.8"))  # 기본 3.8% (ChatGPT 권장)

# ── 거래대금 절대값 필터 (ChatGPT 자문 반영) ─────────────────────────────────
# 당일 거래대금이 이 금액 미만이면 진입 차단
# 기본 30억 — 소형주 급락 방지, 슬리피지 최소화
MIN_TRADING_VALUE = int(os.getenv("MIN_TRADING_VALUE", "3000000000"))

# ── 갭상승 필터 (ChatGPT 자문 반영) ──────────────────────────────────────────
# 시초가가 전일 종가 대비 이 비율 이상 갭상승한 종목 차단
# 기본 3.5% — 갭상승 후 되돌림 손절 방지
GAP_UP_FILTER_ENABLED = os.getenv("GAP_UP_FILTER_ENABLED", "true").lower() == "true"
MAX_GAP_UP_PCT        = float(os.getenv("MAX_GAP_UP_PCT", "3.5"))
...
def _calc_rsi(closes: list[float], period: int = 14) -> Optional[float]:
    ...
    [v4] 최소 period×3 데이터 요구 → SMA 시드 영향 희석, HTS 값에 근접
    """
...
async def check_breakout(
    ...
    """
    돌파 조건 + 기술지표 필터 + ATR + 거래대금 절대값 + 갭상승 필터 적용.
    ...
    # ── 거래대금 절대값 필터 (ChatGPT 자문 반영) ──────────────────────────────
    # 당일 거래대금이 MIN_TRADING_VALUE 미만이면 즉시 차단
    # 배수 조건(VOLUME_MULTIPLIER)과 다름 — 평균이 낮으면 배수를 채워도 절대량이 부족할 수 있음
    if MIN_TRADING_VALUE > 0 and today.trading_value < MIN_TRADING_VALUE:
        logger.debug(
            f"[{code}] {name} 거래대금 부족: "
            f"{today.trading_value/1e8:.1f}억 < {MIN_TRADING_VALUE/1e8:.0f}억 — 차단"
        )
        return None

    # ── 갭상승 필터 (ChatGPT 자문 반영) ──────────────────────────────────────
    # 시초가가 전일 종가 대비 MAX_GAP_UP_PCT 이상 갭상승한 종목 차단
    # 갭상승 후 되돌림은 자동매매 손절의 대표 패턴
    if GAP_UP_FILTER_ENABLED and today.open and len(history) > 0:
        prev_close = history[0].close  # 전일 종가
        if prev_close and prev_close > 0:
            gap_up_pct = (today.open - prev_close) / prev_close * 100
            if gap_up_pct > MAX_GAP_UP_PCT:
                logger.debug(
                    f"[{code}] {name} 갭상승 차단: "
                    f"시초가 갭 +{gap_up_pct:.1f}% > {MAX_GAP_UP_PCT}%"
                )
                return None
    ...
    if FILTER_MACD_ENABLED and macd_data["histogram"] is not None:
        if macd_data["histogram"] <= 0:
            failed_filters.append(f"MACD 히스트 {macd_data['histogram']:.4f} ≤ 0")
    ...
    if ATR_FILTER_ENABLED and atr_pct is not None:
        if atr_pct > ATR_FILTER_MAX_PCT:
            failed_filters.append(
                f"ATR {atr_pct:.2f}% > {ATR_FILTER_MAX_PCT}% (고변동성)"
            )
    ...
    reason_parts = [
        ...
        f"거래대금 {today.trading_value/1e8:.0f}억({today.trading_value/avg_value:.1f}배)",
        f"등락률 {today.change_rate:.1f}%",
    ]
    if rsi is not None:
        reason_parts.append(f"RSI {rsi:.1f}")
    if macd_data["histogram"] is not None:
        reason_parts.append(f"MACD {macd_data['histogram']:+.4f}")
    ...
    logger.info(
        ...
        f"거래대금:{today.trading_value/1e8:.0f}억"
    )
